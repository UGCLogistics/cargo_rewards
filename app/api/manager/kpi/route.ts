...
export async function GET(request: Request) {
  try {
    ...
    // Ambil semua transaksi (hanya user_id + date), lalu hitung last date per user di Node
    ...
    // Map user_id -> last_transaction_date (string YYYY-MM-DD)
    const lastMap = new Map<string, string>();

    for (const row of (lastTxData ?? []) as any[]) {
      const userId = row.user_id as string;
      const dateStr = row.date as string; // kolom "date" di tabel transactions

      if (!userId || !dateStr) continue;

      const prev = lastMap.get(userId);
      // ambil tanggal paling akhir (string YYYY-MM-DD aman dibanding lexicographically)
