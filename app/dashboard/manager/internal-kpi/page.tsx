...
export default function ManagerInternalKpiPage() {
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      // Fetch detail data for manager
      const detailRes = await fetch(`/api/manager/kpi/detail?${params.toString()}`);
      ...
      const kpiRes = await fetch(`/api/manager/kpi?${params.toString()}`);
      const kpiJson = await kpiRes.json();
      if (!kpiRes.ok) throw new Error(kpiJson.error || 'Gagal memuat KPI');
      setKpiTotals(kpiJson.data || null);
      // Aggregate by date for chart, including cashback
  ...
  return (
    <DashboardLayout>
      <h1>Dashboard KPI Internal (Manager)</h1>
      {/* Filters */}
