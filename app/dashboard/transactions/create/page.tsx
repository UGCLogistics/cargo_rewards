 ...
 * transaction data. Hanya bisa diakses oleh role INTERNAL
 * (ADMIN / MANAGER / STAFF). CUSTOMER akan diarahkan ke
...
export default function CreateTransactionPage() {
  ...
  // Redirect bila bukan internal
  useEffect(() => {
    if (!user) return; // biarkan dulu sampai auth ready
    if (!isInternal) {
      router.replace("/dashboard/transactions");
    }
  }, [user, isInternal, router]);

  // Sambil redirect, tampilkan pesan singkat
  if (!user || !isInternal) {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold">Buat Transaksi Baru</h1>
      <form onSubmit={handleSubmit} style={{ maxWidth: "600px" }}>
        ...
        <div style={{ marginBottom: "1rem" }}>
          <label htmlFor="publishRate">Tarif Publish (Rp)</label>
          <br />
          <input
            id="publishRate"
            type="number"
